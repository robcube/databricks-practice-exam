...
export class ContentSpecificQuestionService {
   ...
   */
  async generateProductionPipelineQuestions(): Promise<Question[]> {
...
def critical_business_metrics():
   ...
   */
  async generateIncrementalDataProcessingQuestions(): Promise<Question[]> {
    const questions: QuestionCreateRequest[] = [
...
WHEN NOT MATCHED AND s.operation IN ('I', 'U') THEN
   ...
   */
  async validateContentCoverage(topic: ExamTopic): Promise<ContentCoverageValidation> {
    ...
    if (topic === 'Production Pipelines') {
      ...
      const hasDeltaliveTables = questions.some(q => 
        q.subtopic.toLowerCase().includes('delta live tables') ||
      ...
      const hasJobScheduling = questions.some(q => 
        q.subtopic.toLowerCase().includes('job scheduling') ||
      ...
      const hasErrorHandling = questions.some(q => 
        q.subtopic.toLowerCase().includes('error handling') ||
        q.questionText.toLowerCase().includes('error') ||
        q.tags.includes('error-handling')
      );

      const hasPipelineOrchestration = questions.some(q => 
        q.questionText.toLowerCase().includes('pipeline') ||
        q.tags.includes('pipeline-orchestration')
      );

      const hasDataQuality = questions.some(q => 
        q.questionText.toLowerCase().includes('expectation') ||
        q.questionText.toLowerCase().includes('quality') ||
        q.tags.includes('data-quality')
      );

      if (!hasDeltaliveTables) missingScenarios.push('Delta Live Tables');
      if (!hasJobScheduling) missingScenarios.push('Job Scheduling');
      if (!hasErrorHandling) missingScenarios.push('Error Handling');
      if (!hasPipelineOrchestration) missingScenarios.push('Pipeline Orchestration');
      if (!hasDataQuality) missingScenarios.push('Data Quality Expectations');
      ...
      const hasMergeOperations = questions.some(q => 
        q.subtopic.toLowerCase().includes('merge') ||
        q.questionText.toLowerCase().includes('merge') ||
        q.codeExample?.toLowerCase().includes('merge') ||
        q.tags.includes('merge-operations')
      );

      const hasCDC = questions.some(q => 
        q.subtopic.toLowerCase().includes('change data capture') ||
      ...
      const hasStreaming = questions.some(q => 
        q.subtopic.toLowerCase().includes('streaming') ||
        q.questionText.toLowerCase().includes('stream') ||
        q.codeExample?.toLowerCase().includes('readstream') ||
        q.tags.includes('streaming')
      );

      const hasWatermarking = questions.some(q => 
        q.questionText.toLowerCase().includes('watermark') ||
        q.codeExample?.toLowerCase().includes('watermark') ||
        q.tags.includes('watermarking')
      );

      const hasExactlyOnce = questions.some(q => 
        q.questionText.toLowerCase().includes('exactly-once') ||
        q.questionText.toLowerCase().includes('exactly once') ||
        q.tags.includes('exactly-once')
      );

      if (!hasMergeOperations) missingScenarios.push('Merge Operations');
      if (!hasCDC) missingScenarios.push('Change Data Capture');
      if (!hasStreaming) missingScenarios.push('Streaming Processing');
      if (!hasWatermarking) missingScenarios.push('Watermarking');
      if (!hasExactlyOnce) missingScenarios.push('Exactly-Once Semantics');
    }
   ...
   */
  async generateQuestionsWithDifficultyVariation(
    topic: ExamTopic,
    totalQuestions: number,
    distribution?: DifficultyDistribution
  ): Promise<Question[]> {
    // Default distribution: 30% easy, 50% medium, 20% hard
    const defaultDistribution: DifficultyDistribution = {
      easy: 0.3,
      medium: 0.5,
      hard: 0.2
    };
   ...
   */
  async getComprehensiveContentCoverageReport(): Promise<ContentCoverageValidation[]> {
    const topics: ExamTopic[] = ['Production Pipelines', 'Incremental Data Processing'];
    
    const coverageReports = await Promise.all(
      topics.map(topic => this.validateContentCoverage(topic))
    );
   ...
   */
  async initializeContentSpecificQuestions(): Promise<{
