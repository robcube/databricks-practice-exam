...
function validateMigrationFiles() {
    ...
    try {
        ...
        const migrations = files.map(filename => {
            const content = (0, fs_1.readFileSync)((0, path_1.join)(migrationsDir, filename), 'utf8');
...
function validateSQLSyntax(migration) {
...
function validateTableRelationships(migrations) {
...
function validateRequiredTables(migrations) {
    const allContent = migrations.map(m => m.content).join('\n').toLowerCase();
...
function extractTableNames(migrations) {
    const allContent = migrations.map(m => m.content).join('\n');
    const createTablePattern = /CREATE TABLE(?:\s+IF NOT EXISTS)?\s+(\w+)/gi;
