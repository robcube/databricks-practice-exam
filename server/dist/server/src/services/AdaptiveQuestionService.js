...
exports.AdaptiveQuestionService = void 0;
...
class AdaptiveQuestionService {
     ...
     */
    async analyzeUserPerformance(examHistory, config = {}) {
        ...
        const recommendedAllocation = this.calculateQuestionAllocation(topicScores, weakAreas, strongAreas, finalConfig);
        return {
            weakAreas,
            strongAreas,
            topicScores,
     ...
     */
    async generateAdaptiveQuestionSet(examHistory, config = {}) {
        ...
        return await this.generateWeightedQuestionSet(performanceAnalysis.recommendedAllocation, finalConfig.totalQuestions);
    }
    /**
     * Calculates performance trends over time
     * Implements Requirement 3.2
     */
    calculatePerformanceTrends(examHistory) {
     ...
     */
    generateStudyRecommendations(performanceAnalysis, trends) {
     ...
     */
    shouldReduceTopicAllocation(topic, examHistory) {
    ...
    getRecentResults(examHistory, count) {
        return examHistory
            .sort((a, b) => b.endTime.getTime() - a.endTime.getTime())
            .slice(0, count);
    }
    calculateAverageTopicScores(examResults) {
    ...
    calculateQuestionAllocation(topicScores, weakAreas, strongAreas, config) {
        const allocations = [];
        const allTopics = [
            'Databricks Lakehouse Platform',
            'ELT with Spark SQL and Python',
    ...
    async generateWeightedQuestionSet(allocations, totalQuestions) {
        const selectedQuestions = [];
        for (const allocation of allocations) {
            if (allocation.questionCount > 0) {
                // Get more questions than needed to allow for random selection
                const candidateQuestions = await this.questionRepository.findByTopic(allocation.topic, allocation.questionCount * 2);
                // For weak areas, prefer medium and hard questions
                let filteredQuestions = candidateQuestions;
                if (allocation.priority === 'high') {
                    const challengingQuestions = candidateQuestions.filter(q => q.difficulty === 'medium' || q.difficulty === 'hard');
    ...
    getBalancedDistributionForNewUser(config) {
        const allTopics = [
            'Databricks Lakehouse Platform',
            'ELT with Spark SQL and Python',
            'Incremental Data Processing',
        ...
        const recommendedAllocation = allTopics.map((topic, index) => ({
            topic,
    ...
    getTopicScoreFromExam(exam, topic) {
    ...
    calculateTrendDirection(scores) {
        ...
        if (Math.abs(difference) < 5)
            return 'stable'; // Less than 5% change is considered stable
        return difference > 0 ? 'improving' : 'declining';
    }
    getTopicSpecificFocusAreas(topic, currentScore) {
        const focusAreas = [];
        switch (topic) {
...
exports.AdaptiveQuestionService = AdaptiveQuestionService;
//# sourceMappingURL=AdaptiveQuestionService.js.map