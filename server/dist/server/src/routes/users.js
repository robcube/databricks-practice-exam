...
const validateStudyGoal = [
    ...
    (0, express_validator_1.body)('targetScore').isInt({ min: 0, max: 100 }).withMessage('Target score must be between 0 and 100'),
    (0, express_validator_1.body)('deadline').optional().isISO8601().withMessage('Deadline must be a valid date')
];
 ...
 */
router.get('/:userId', validateUserId, handleValidationErrors, auth_1.authenticateToken, (req, res) => {
    try {
 ...
 */
router.get('/:userId/exam-history', validateUserId, handleValidationErrors, auth_1.authenticateToken, [
    ...
    try {
        ...
        if (examType) {
            examHistory = examHistory.filter((exam) => exam.examType === examType);
        }
        // Apply pagination
 ...
 */
router.post('/:userId/exam-results', validateUserId, handleValidationErrors, auth_1.authenticateToken, (req, res) => {
    try {
        const { userId } = req.params;
        const requestingUserId = req.userId;
        ...
        const examResult = new ExamResult_1.ExamResultModel(req.body);
 ...
 */
router.get('/:userId/study-goals', validateUserId, handleValidationErrors, auth_1.authenticateToken, (req, res) => {
    try {
        const { userId } = req.params;
        const requestingUserId = req.userId;
 ...
 */
router.post('/:userId/study-goals', validateUserId, validateStudyGoal, handleValidationErrors, auth_1.authenticateToken, (req, res) => {
    try {
        const { userId } = req.params;
        const requestingUserId = req.userId;
        ...
        const existingGoal = user.studyGoals.find((goal) => goal.topic === topic);
        if (existingGoal) {
...
router.put('/:userId/study-goals/:goalId', validateUserId, [
    (0, express_validator_1.param)('goalId').isLength({ min: 1 }).withMessage('Goal ID is required'),
    (0, express_validator_1.body)('targetScore').optional().isInt({ min: 0, max: 100 }).withMessage('Target score must be between 0 and 100'),
    (0, express_validator_1.body)('deadline').optional().isISO8601().withMessage('Deadline must be a valid date'),
    // body('isCompleted').optional().isBoolean().withMessage('isCompleted must be a boolean')
], handleValidationErrors, auth_1.authenticateToken, (req, res) => {
    try {
        ...
        const goalIndex = user.studyGoals.findIndex((goal) => goal.id === goalId);
        if (goalIndex === -1) {
            return res.status(404).json({
 ...
 */
router.get('/:userId/performance-summary', validateUserId, handleValidationErrors, auth_1.authenticateToken, (req, res) => {
    try {
        const { userId } = req.params;
        const requestingUserId = req.userId;
