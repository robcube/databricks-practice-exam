...
const ExamEngineService_1 = require("../services/ExamEngineService");
const QuestionService_1 = require("../services/QuestionService");
const router = express_1.default.Router();
const examEngine = new ExamEngineService_1.ExamEngineService();
...
router.post('/', async (req, res) => {
    try {
        ...
        const questions = await questionService.getQuestionsForExam(examType, questionCount);
        if (questions.length === 0) {
            return res.status(500).json({
                error: 'No questions available for exam'
            });
        }
        const session = examEngine.startExamSession(userId, examType, questions);
        res.status(201).json({
...
//# sourceMappingURL=exam-sessions.js.map